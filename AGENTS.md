...
| First-fill without a terms-change question | Filling an empty amount/cadence/plan, or clearing one back to unknown, saves as ordinary completion, in the edit form and on a capture card alike — a price for a row that had none raises an `update`, not a `terms_changed`. Only replacing a recorded term asks correction-vs-change, showing old → new; an actual change still needs the user's effective date and versions history. The explicit change action stays available when the old terms were never recorded | [SUB-58](https://linear.app/lets-play-match/issue/SUB-58/add-missing-subscription-terms-without-a-terms-change-question) |
| Holding identity rule | `resolveCandidate` matches on provider **and** account (`matched` / `ambiguous` / `unseen_account` / `none`); several compatible holdings or an unseen account raise an `account_identity` question naming the holdings instead of picking one; a repeated pending `create` for the same provider+account folds onto the existing draft; `capture_questions` are keyed by `(user_id, scope_key, reason)` with `holding:<id>` / `draft:<provider>\|<account>` scopes; accept rechecks under the user lock and returns `409 duplicate_holding` (an equivalent draft already became a holding) or `409 stale_target` (the holding's provider/account changed since the card was raised). Journeys in `lib/journeys/identity.integration.test.ts` | [SUB-52](https://linear.app/lets-play-match/issue/SUB-52/decide-the-holding-identity-rule-for-capture) |
| Shared Work/Subscriptions responsive shell | One `/workspace` route holds the conversation, the **Work** and **Subscriptions** views, and the record open beside them; `/`, `/inbox`, `/ledger`, and `/ledger/<id>` redirect into it carrying their params, so an old link still reaches what it named. View, open record, narrow-screen pane, composer target, and inventory filters all live in the URL. Work is one card per holding carrying every reason it is there, with its questions folded onto it; inventory opens on **All** | [SUB-62](https://linear.app/lets-play-match/issue/SUB-62/bring-work-and-subscriptions-into-one-responsive-workspace) |
| Subscriptions as the primary workspace | The `/workspace` list joins the ledger page, pending cards and inbox sections by stable holding id on the client (`lib/workspace/subscription-list.ts`); drafts without a holding are their own rows marked **Not added yet**, and two drafts of one provider are never merged. `show=all|reviews|questions|reminders` plus `record=` or `draft=` live in the URL; `view=work` and the old `/inbox`, `/ledger`, `/ledger/<id>` links redirect into them. An open row keeps showing after an action removes it from the filter until it is closed. Inventory totals and coverage are gone from the surface; `GET /api/subscriptions/summary` is unchanged | [SUB-65](https://linear.app/lets-play-match/issue/SUB-65/make-subscriptions-the-primary-workspace-with-contextual-reviews) |
| Provider correction on a card | A pending `create` card offers the holdings its provider resembles (`likelyMatches`) and a "Wrong provider or account?" correction; `POST /api/proposals/[id]/retarget` accepts `{useExisting: <id>}` or corrections `{provider, plan, accountHint}` and re-runs `resolveCandidate` — a corrected name that lands on one holding retargets the draft in place (same card id, fresh `target`, the heard name kept in the rationale); a `duplicate` question answered "same" retargets the draft it was asked about, "new" keeps it as its own holding. Retargeting recomputes the proposal kind and delta and never confirms money, dates, auto-renewal, or reminder consent; it never merges or deletes a created holding. Journeys in `lib/journeys/identity.integration.test.ts` | [SUB-56](https://linear.app/lets-play-match/issue/SUB-56/a-misread-provider-cannot-be-corrected-on-a-card-so-it-becomes-a-new) |
| Simple-invoice cost and billed-period inference | Extraction carries a stated `servicePeriod` apart from `nextRenewal` and `paidOn`; `lib/capture/service-period.ts` reads only a complete current one-calendar-month period as `inferred` monthly with its end as the `inferred` next due, and the proposal rationale states the continuation assumption. Invoice amounts are net plus stated VAT; adjusted totals are not read. Confirmed cadence and renewal are untouched; explicit `nextRenewal` and `paidOn` inference keep precedence | [SUB-68](https://linear.app/lets-play-match/issue/SUB-68/propose-tax-inclusive-invoice-cost-and-inferred-renewal-from-a-clear) |
