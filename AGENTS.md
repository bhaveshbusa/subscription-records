...
- **Expected next renewal** is a separate labelled value. After [SUB-48](https://linear.app/lets-play-match/issue/SUB-48/show-expected-renewals-and-remove-routine-confirmation-work) it may appear beside the recorded date. Until that issue ships, `main` still exposes only the stored date — do not implement the projection in another PR.
- Project an expected date only when status is `active`, auto-renewal is **confirmed yes**, cadence is **confirmed**, and the recorded date is **confirmed**. Label the result inferred/expected. Do not write it back.
...
| Independent reminder preferences | Preference table, manual controls, date preview; unset ≠ off; cadence does not overwrite a stored choice. Capture still does not write these until SUB-45. Inbox still uses Renewing soon until SUB-49. | [SUB-47](https://linear.app/lets-play-match/issue/SUB-47/save-independent-reminder-preferences) |
...
| Paid-commitment totals exclude trials; coverage/omission | Summary includes trial GBP amounts | [SUB-46](https://linear.app/lets-play-match/issue/SUB-46/explain-spend-coverage-and-separate-trials-from-paid-commitments) |
