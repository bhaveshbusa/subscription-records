...
function systemPrompt(today: string, source: Source): string {
  return [
    ...
    "Imperative cancel is a cancellation with unknown timing, not a wish: \"Cancel subscription\", \"Cancel Netflix\", or \"cancel it\" (with a named or selected service) sets `lifecycle` to `cancelled` and leaves `endsOn` null. That is different from \"I want to cancel\" / \"I should cancel\".",
    ...
    "Quote the words the candidate came from in `evidence`, including the cancellation words and any timing (\"three months ago\", \"in March\", \"Cancel subscription\") when there are any, since the date is read from them.",
