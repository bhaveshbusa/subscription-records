...
- **Accept confirms proposal terms** ([SUB-87](https://linear.app/lets-play-match/issue/SUB-87/accept-confirms-proposal-terms-and-remove-per-field-confirm-on-cards)). Accepting a terms proposal confirms exactly the money/date/auto-renewal values on that proposal after Edits. Status/lifecycle accepts remain distinct and do not confirm money. Labels keep the status/terms difference clear. Per-field Confirm staging is removed from proposal cards; keep Edit + Accept / Reject. Saved-row Confirm may remain for legacy proposed leftovers.
