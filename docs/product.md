...
The contract in this file and [AGENTS.md](../AGENTS.md) is the definition of correct for the shipped product including SUB-66–SUB-69. [User journeys](user-journeys.md) describe motivations and interaction decisions; [testing-and-signoff.md](testing-and-signoff.md) provides reusable checks. Bhavesh signed off the project on 13 September 2026; the [validation record](validation/sub-63-run-2026-09-11.md) records the evidence and limits.
...
- A simple invoice — one service, stated net and tax, a clear billing period — is evidence of the current terms *(shipped — [SUB-68](https://linear.app/lets-play-match/issue/SUB-68/propose-tax-inclusive-invoice-cost-and-inferred-renewal-from-a-clear))*. The recurring cost it proposes is tax-inclusive: net plus VAT, never an amount due after credits, balances, or one-off adjustments. A complete one-calendar-month period that today falls inside reads as monthly, labelled inferred, with the period end as the inferred next due if the service continues; the card's rationale says so. The invoice's issue date and due date are not the next renewal and not a payment date. An ambiguous, historical, future, or prorated period is carried as written and asked about, never turned into a cadence or a date. A billed period does not confirm auto-renewal and does not mean the bill was paid. Multi-line tax allocation is not attempted.
...
- Do not auto-confirm `amount`, `cadence`, `next_renewal`, trial end, or auto-renewal from time passage, opening a form, or an unrelated field. Do not overwrite confirmed money/date fields (write `terms_changed` or mark `conflicted`). Do not delete subscription identity on cancel.
- Cadence never confirms auto-renewal unless auto-renewal is itself on the accepted proposal. Auto-renewal is yes, no, or unknown, established from the user or evidence.
- **Accept confirms** ([SUB-82](https://linear.app/lets-play-match/issue/SUB-82/publish-accept-confirms-and-pending-terms-latest-wins-decisions); implement in [SUB-87](https://linear.app/lets-play-match/issue/SUB-87/accept-confirms-proposal-terms-and-remove-per-field-confirm-on-cards)). Accepting a terms proposal confirms exactly the money/date/auto-renewal values present on that proposal after any Edits. Prefer: values shown on the card become `confirmed` on Accept; Edit or Reject if they are wrong. Status-only and lifecycle accepts still do not confirm money.
- **Pending terms latest-wins** ([SUB-82](https://linear.app/lets-play-match/issue/SUB-82/publish-accept-confirms-and-pending-terms-latest-wins-decisions); implement in [SUB-88](https://linear.app/lets-play-match/issue/SUB-88/supersede-pending-terms-on-one-holding-with-latest-wins-and-fix)). On the same holding, a newer pending terms proposal supersedes prior pending field values it names. Multiple pending terms cards coalesce (latest field wins); evidence is retained; the user Edits to correct. Lifecycle cancel/reactivate stays separate. Never merge holdings. Latest-wins is pending versus pending only — saved confirmed ledger fields still use terms-change / conflict rules.
...
### Precise field review *(first-fill landed in [SUB-58](https://linear.app/lets-play-match/issue/SUB-58/add-missing-subscription-terms-without-a-terms-change-question); per-field controls landed in [SUB-59](https://linear.app/lets-play-match/issue/SUB-59/confirm-and-edit-individual-fields-directly-on-proposals-and-records); Accept-confirms decision in [SUB-82](https://linear.app/lets-play-match/issue/SUB-82/publish-accept-confirms-and-pending-terms-latest-wins-decisions))*
...
- **Accept confirms proposal terms.** Accepting a terms proposal confirms exactly the money/date/auto-renewal values on that proposal after Edits. Status/lifecycle accepts remain distinct and do not confirm money. Labels keep the status/terms difference clear.
- **Follow-on UI ([SUB-87](https://linear.app/lets-play-match/issue/SUB-87/accept-confirms-proposal-terms-and-remove-per-field-confirm-on-cards)):** remove per-field Confirm staging on proposal cards; keep Edit + Accept / Reject. Saved-row Confirm may remain for legacy proposed leftovers. Until that issue lands, shipped staging UI may still exist — the product decision above is authoritative for new writers.
...
- **First-fill versus change.** This applies to a capture card as much as to the edit form: a message that gives a price to a row that had none is an **Update**, not a Terms changed. Filling an empty amount/cadence/plan is ordinary completion — no correction-versus-terms-change interrogation. Clearing a term back to unknown is not one either: nothing is superseded, and there is no date on which "unknown" took effect. Replacing a known term shows old → new and asks whether it is a correction or an actual change; an actual change needs the user's effective date and versions history through the shared writer. The explicit change action remains available even when old terms were never recorded — the earlier period is closed the day before the new terms start rather than overwritten.
...
- **Pending terms coalesce ([SUB-88](https://linear.app/lets-play-match/issue/SUB-88/supersede-pending-terms-on-one-holding-with-latest-wins-and-fix)).** Multiple pending terms proposals on one holding fold with latest field value wins; evidence retained; lifecycle proposals stay separate; holdings are never merged.
