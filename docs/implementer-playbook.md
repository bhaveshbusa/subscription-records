...
Attach the PR to the Linear issue **in Linear**. This team has no `In Review` state, and `Done` is the human's after sign-off. Do not merge. Wait for the human. Do not record issue status in git docs.
